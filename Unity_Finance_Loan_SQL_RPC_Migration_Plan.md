# Unity Finance — Exact Loan SQL / RPC Migration

**Migration target:** existing Unity Finance MVP Supabase schema  
**Recommended migration filename:** `20260902_loan_domain_hardening_and_rpc.sql`  
**Prerequisite:** the existing `Unity_Finance_MVP_Supabase_Schema.sql` has already been applied to the development Supabase project.

> **Important:** this migration is intentionally written against the existing MVP schema. It does not recreate the entire database. It upgrades/hardens the loan domain, adds the missing loan support tables, removes unsafe direct client writes, adds controlled RPCs, and adds idempotent scheduled processors.

The playbook is the business source of truth. Where it does not define an exact formula or policy, this migration deliberately avoids inventing one. Those items are listed in the final section.
...
- Member loan: one-time 10% service charge.
- Outsider loan: one-time 15% service charge.
- Initial maximum loan: 20,000 ETB.
- Standard term: 3 months.
- Early repayment: no early-repayment penalty.
- Late installment penalty: 5% of the overdue installment, charged once.
- Serious default: unpaid installment reaches 60 days after its due date.
- Outsider loan requires an active member guarantor.
- Guarantor can have only one outstanding outsider guarantee at a time.
- At least two distinct authorized people must approve a loan.
- No self-approval.
- Final liquidity gate before disbursement.
- Minimum liquidity reserve: 30% of member savings.
- Financial history is append-only from the application's perspective.
- Reversals/corrections, not deletes, for posted financial corrections.

The playbook defines these rules explicitly. 

---

# 2. What this migration changes

### Existing objects hardened
...
- `loan_default_events`
- `loan_disbursements`
- loan-related grants and RLS behavior
...
- `private.loan_liquidity_snapshot()`
- controlled public loan RPCs
- overdue/default scheduled processors
...
select to_regclass('public.loan_repayment_allocations');
select to_regclass('public.savings_security_locks');
select to_regclass('public.loan_extension_requests');
select to_regclass('public.loan_recovery_events');
...
where fr.rule_code in (
  ...
  'serious_default_days',
  'liquidity_reserve_rate'
)
and frv.status = 'active'
order by fr.rule_code;


-- ============================================================
-- F. RLS enabled for new tables
-- ============================================================
...
select
  ...
  command
from cron.job
where jobname like 'unity-finance-loan-%';
```

---

# 5. Recommended RLS tests

The Supabase project should also contain database tests for allow/deny behavior.

At minimum verify:

```text
ANONYMOUS
  cannot read loan records

MEMBER A
  can read own loans
  can read own applications
  cannot read Member B loans
  cannot insert loan approvals
  cannot update loan status
  cannot disburse a loan

GUARANTOR
  can read their assigned guarantee
  can accept/reject through RPC
  cannot directly update the guarantee

ADMIN / APPROVER
  can read permitted loan data
  can execute loan review RPC
  can execute disbursement RPC if permission exists

FINANCE / REPAYMENT ACTOR
  can post verified repayments through RPC
  cannot directly insert ledger entries
```

Supabase recommends testing both allow and deny paths for exposed tables and RLS policies. citeturn328329search3turn328329search4

---

# 6. Important security notes

This migration deliberately uses `SECURITY DEFINER` only for controlled privileged functions and pins `search_path = ''` while schema-qualifying database objects. Supabase's current guidance recommends `SECURITY INVOKER` by default and requires special care, including a pinned search path and restricted function execution, for `SECURITY DEFINER`. citeturn328329search2

The migration also removes direct authenticated-user writes to loan state tables. RLS is still enabled for reads, but financial state transitions are performed through controlled RPCs. Supabase emphasizes that grants and RLS are separate controls and that RLS does not protect function execution, so function `EXECUTE` privileges must also be restricted. citeturn328329search4

---

# 7. Cron

The overdue/default processors are intentionally idempotent.

Supabase Cron runs recurring Postgres jobs through `pg_cron` and can invoke database functions directly. citeturn328329search0turn328329search6

The migration schedules:

```text
00:15 UTC → overdue installment processor
00:30 UTC → serious-default processor
```

Because Ethiopia is UTC+3, these run at:
...
The business logic is daily rather than tied to a single exact wall-clock moment, which makes retries and missed runs much safer.

---

# 8. Important policy decisions intentionally NOT invented

These must still be approved by Unity Finance before production:

## A. Repayment capacity formula

The playbook says loan amount depends partly on repayment capacity, but it does not define:
...
Therefore approval requires an explicit manual confirmation:
...
## B. Guarantor security ratio

The playbook requires sufficient savings/security but does not define how much.
...
for outsider approval rather than inventing a percentage.

## C. Payment allocation priority

The playbook does not define whether a combined payment should settle:

```text
penalty first
or installment first
```

Therefore the repayment RPC takes explicit allocations.
...
This is deliberate. The software should not secretly invent accounting policy.

## D. Extension schedule behavior

The playbook permits extension but doesn't define how subsequent installments are rewritten.

The migration therefore stores the request but does not automatically rewrite the schedule.

## E. Recovery priority

The playbook permits recovery from savings/security and guarantors but doesn't define an exact enforcement order.

No automatic forced recovery is implemented.

---

# 9. One implementation choice that must be understood

The current migration recognizes the loan service charge **at disbursement**:

```text
Loan Receivable       DEBIT  total repayment
Cash/Bank/Wallet      CREDIT principal
Service Charge Income CREDIT service charge
```
...
Contractual receivable    11,000
Cash released             10,000
Income                     1,000
```

This makes the loan receivable equal to the contractual repayment amount and allows repayment to reduce that receivable.

If your accountant wants service-charge recognition on a different basis, this is one place that should be changed before production.

---

# 10. Migration safety

Apply this first to the **development Supabase project**.

Do not run it against production until:

```text
1. Schema verification passes.
2. RPC security tests pass.
3. RLS tests pass.
4. Two-person approval is tested.
5. Duplicate-disbursement test passes.
6. Duplicate-repayment test passes.
7. Late-penalty idempotency test passes.
8. 60-day default test passes.
9. Liquidity gate is reconciled against actual group accounting.
10. A business/finance owner signs off on the unresolved policy items.
```

For deployment, keep this as a real Supabase migration rather than manually pasting it into production. Supabase recommends keeping grants, RLS and schema changes in reproducible migrations. citeturn328329search3
